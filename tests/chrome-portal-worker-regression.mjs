...
function fixture(initial = ['https://portal.test/*']) {
  ...
  const runtime = { id: 'extension-id', getManifest: () => ({ version: '7.5.114' }), getURL: file => `chrome-extension://extension-id/${file}`, onMessage: event(), onInstalled: event(), onStartup: event() };
    ...
    executeScript: async args => {
      ...
      if (args.files) {
        ...
        if (!state.silentFileError) Object.assign(realms.get(`${args.target.documentIds[0]}:MAIN`), {
          ...
          __PENA_DIALOG_REPOSITORY__: { get() {} }, __ANITREC_RUNNING__: '7.5.114',
        });
