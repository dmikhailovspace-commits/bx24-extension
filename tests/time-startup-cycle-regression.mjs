...
fixture=fixture.replace(scriptAnchor,`<script>
 (()=>{
  ...
  const state=window.startupCycleServer={mode,repository,clockFailure,startedAt:performance.now(),calls:[],repositoryCalls:0,repositoryResolvedAt:0,identityReleased:mode==='early',identityReleasedAt:mode==='early'?performance.now():0,heldIdentity:[],physicalCompletedAt:0,readyAt:0,totalAt:0};
  ...
  const sample=()=>{
   if(window.__resumeHarness.sourceComplete('chats')&&!state.physicalCompletedAt)state.physicalCompletedAt=performance.now();
   if(window.__resumeHarness.ready('chats')&&!state.readyAt)state.readyAt=performance.now();
   if(window.startupCycleProbe?.snapshot().complete&&!state.totalAt)state.totalAt=performance.now();
   if(mode==='after-physical'&&state.physicalCompletedAt)state.releaseIdentity();
...
try{
 const cases=[{name:'early',identityMode:'early'},{name:'delayed',identityMode:'delayed'},{name:'after-physical',identityMode:'after-physical'},
 ...
 for(const scenario of cases.filter(item=>!process.env.PENA_STARTUP_CYCLE_CASE||process.env.PENA_STARTUP_CYCLE_CASE.split(',').includes(item.name))){
  ...
  try{
   ...
   await page.waitForFunction(()=>__resumeHarness.ready('chats')&&startupCycleProbe.snapshot().complete,undefined,{timeout:scenario.clockFailure?45000:25000});
   ...
   assert.equal(ready.native.modes.chats.sourceComplete,true);assert.equal(ready.native.modes.chats.observedIds.length,108);
   assert.ok(ready.native.modes.chats.poolSize<108);assert.ok(ready.native.modes.chats.reusePasses>0);
   ...
   if(identityMode==='after-physical')assert.ok(ready.server.identityReleasedAt>=ready.server.physicalCompletedAt);
   ...
   if(scenario.repository==='late-tail'){
    await page.waitForFunction(()=>__resumeHarness.ready('chats')&&__resumeHarness.state().modes.chats.observedIds.includes('chat991999'),undefined,{timeout:20000});
   }
   ...
   if(scenario.repository==='late-tail'){assert.equal(settled.native.modes.chats.observedIds.length,109);assert.ok(settled.native.modes.chats.catalogIds.includes('chat991999'));}
