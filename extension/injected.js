	...
	// URL логотипа передаётся через data-атрибут тега <script> из content.js
	const _PENA_LOGO_URL = (function() {
		try { return document.currentScript?.dataset?.logoUrl || ''; } catch(e) { return ''; }
	})();
  ...
  <svg viewBox="0 0 402.577 402.577" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
	...
	host.addEventListener('mousedown', (ev) => {
		...
		const onRzMove = (e) => {
			...
			if (_rzEdges.b && pane) {
				pane.style.maxHeight = Math.max(150, Math.min(window.innerHeight * 0.95, _rzStartH + dy)) + 'px';
			}
		};
		const onRzUp = () => {
			_rzActive = false;
