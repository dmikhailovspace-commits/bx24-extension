	...
	window.__ANITREC_RUNNING__ = '7.1.43';
	...
	async function _setDialogControlReadFlag(dialogId, chatId = null) {
		const id = normId(dialogId);
		const numericChatId = Number(chatId);
		const primaryAttempts = [];
		const fallbackAttempts = [];
		let lastError = null;
		if (Number.isFinite(numericChatId)) {
			primaryAttempts.push(() => _runBitrixAction('im.v2.Chat.read', { chat: numericChatId }));
		}
		if (id) {
			primaryAttempts.push(() => _callBxRestMethod('im.recent.unread', { DIALOG_ID: id, ACTION: 'N' }));
		}
		if (primaryAttempts.length) {
			try {
				await _runFirstSuccessfulDialogControlAttempt(primaryAttempts);
		...
		if (Number.isFinite(numericChatId)) {
			fallbackAttempts.push(() => _runBitrixAction('im.v2.Chat.read', { chat: String(numericChatId) }));
			fallbackAttempts.push(() => _runBitrixAction('im.v2.Chat.read', { chatId: numericChatId }));
			fallbackAttempts.push(() => _runBitrixAction('im.v2.Chat.read', { chat_id: numericChatId }));
		}
		for (const attempt of fallbackAttempts) {
			try {
				await attempt();
...
if (_presetChannel) {
	...
	function _normalizeDialogControlMoveIds(ids) {
		const raw = ids instanceof Set ? Array.from(ids) : (Array.isArray(ids) ? ids : [ids]);
	...
	function _moveDialogControlItemsRelative(movedIds, targetId, side = 'before') {
		...
		if (!moved.length || moved.length !== ids.length || !target || idSet.has(String(target.id))) return false;
	...
	function _moveDialogControlItemsToFolder(movedIds, folderId, atStart = false) {
		...
		const moved = items.filter(item => !_isDialogControlFolder(item) && idSet.has(String(item.id)));
		if (!folder || !moved.length || moved.length !== ids.length) return false;
		const remaining = items.filter(item => !(!_isDialogControlFolder(item) && idSet.has(String(item.id))));
	...
	function _setDialogControlOptimisticRead(dialogId, active) {
		const id = normId(dialogId);
		if (!id) return;
		if (!active) {
			_dialogControlOptimisticRead.delete(id);
		...
		_dialogControlOptimisticLater.delete(id);
		_dialogControlOptimisticRead.set(id, Date.now() + _DIALOG_CONTROL_OPTIMISTIC_READ_MS);
	}

	function _isDialogControlOptimisticRead(dialogId) {
		const id = normId(dialogId);
		const expiresAt = id ? Number(_dialogControlOptimisticRead.get(id)) || 0 : 0;
		if (!expiresAt) return false;
		if (expiresAt < Date.now()) {
			_dialogControlOptimisticRead.delete(id);
			return false;
	...
	function _getDialogControlEffectiveMeta(itemOrId, meta = null) {
		...
		const isClean = !meta?.hasUnread && !meta?.hasLater && !meta?.hasMention && !(Number(meta?.unreadCount) > 0);
	...
	function _applyDialogControlItemDomState(dialogId, h = filtersHost) {
	...
	async function _markDialogControlItemRead(item, h = filtersHost) {
		...
		if (!meta?.hasLater && !meta?.hasUnread && !meta?.hasMention && unreadCount <= 0) {
			_showDialogDockToast('Диалог уже прочитан', 'ok');
			return true;
		}
		const chatId = _getDialogControlChatNumber(item.id, targetEl);
		_setDialogControlOptimisticRead(item.id, true);
		_applyDialogControlItemDomState(item.id, h);
		_setDialogControlReadFlag(item.id, chatId)
			.then(ok => {
				if (!ok) throw new Error('Bitrix API rejected read mark');
				_showDialogDockToast('Диалог отмечен прочитанным', 'ok');
				_refreshDialogControlLaterState(h);
			})
			.catch(() => {
				_setDialogControlOptimisticRead(item.id, false);
				_applyDialogControlItemDomState(item.id, h);
				_dialogControlLastSig = '';
				_scheduleDialogControlPanelRender(h || filtersHost, 0);
				_showDialogDockToast('Не удалось отметить прочитанным через Bitrix API', 'danger');
			});
		return true;
	}
	...
	function _showDialogControlContextMenu(event, item, meta, h = filtersHost) {
		...
		const isRead = !meta?.hasLater && !meta?.hasUnread && !meta?.hasMention && !(Number(meta?.unreadCount) > 0);
		if (isRead) {
			readBtn.classList.add('--active', '--read');
			readBtn.title = 'Диалог уже прочитан';
		}
	...
	function _renderDialogControlPanel(h = filtersHost) {
		...
		const dragPayloadMime = 'application/x-pena-dialog-control-ids';
		...
		const getFirstVisibleFolderChildRow = (folderId) => {
			const id = String(folderId || '');
			const children = getVisibleFolderChildrenMap().get(id) || [];
			return children.length ? children[0] : null;
		};
		const isFirstVisibleFolderChild = (row) => {
			const id = String(row?.dataset?.parentFolderId || '');
			if (!id) return false;
			const children = getVisibleFolderChildrenMap().get(id) || [];
			return !!children.length && children[0] === row;
		};
		...
		const readDragPayloadIds = (event) => {
			const transfer = event?.dataTransfer;
			if (!transfer) return [];
			const values = [];
			try {
				const raw = transfer.getData(dragPayloadMime);
				if (raw) {
					const parsed = JSON.parse(raw);
			...
			try {
				...
				if (raw) values.push(...raw.split(/[,\n]/));
			} catch {}
			return _normalizeDialogControlMoveIds(values);
		};
		const getDragPayloadIds = (event) => {
			const liveIds = _normalizeDialogControlMoveIds(getDraggingIds());
			const transferIds = readDragPayloadIds(event);
			return transferIds.length > liveIds.length ? transferIds : liveIds;
		};
		...
		const canonicalizeDropIntent = (intent) => {
			if (!intent || intent.root || !intent.row) return intent;
			if (draggingType === 'dialog' && intent.side === 'before' && intent.row.dataset?.parentFolderId && isFirstVisibleFolderChild(intent.row)) {
				const folderRow = getFolderRow(intent.row.dataset.parentFolderId);
				if (folderRow && !isDraggingTargetId(folderRow.dataset.folderId || '')) {
					return { ...intent, row: folderRow, side: 'folder-start' };
				}
			}
			return intent;
		};
		const getDropIntentLineBox = (intent) => {
			...
			const side = intent.side || 'before';
			const visualSide = side === 'folder-start' || side === 'folder-after' || side === 'folder-end' ? 'after' : side;
			if (visualSide === 'inside') return { tone: 'inside', id: getDropTargetId(row) };
			const movedItems = draggingType === 'dialog' ? getCurrentDragItems() : [];
			const hierarchyUp = draggingType === 'folder' || side === 'folder-after' || !!(movedItems.some(item => item?.folderId) && side !== 'folder-start' && side !== 'folder-end' && !row.dataset?.parentFolderId);
			const markerRow = side === 'folder-after' && row?.dataset?.folderId
				? (getLastVisibleFolderChildRow(row.dataset.folderId) || row)
		...
		const getDropIntentVisualKey = (intent) => {
			const line = getDropIntentLineBox(intent);
			if (!line) return '';
			if (line.tone === 'inside') return `inside:${line.id || ''}`;
			return `${line.tone}:${Math.round(line.top)}:${Math.round(line.left)}:${Math.round(line.width)}`;
		};
		const getDropRows = () => {
		...
		const getNearestDropTarget = (clientX, clientY) => {
			...
			if (hovered) {
				const intent = canonicalizeDropIntent(getDropIntentForRow(hovered.row, clientY, { gap: false }, hovered.rect));
				return intent ? { ...intent, distance: 0 } : null;
			}
			const byVisualKey = new Map();
			const considerIntent = (intent, distance) => {
				const normalized = canonicalizeDropIntent(intent);
				if (!normalized) return;
				const key = getDropIntentVisualKey(normalized);
				if (!key) return;
				const existing = byVisualKey.get(key);
				if (!existing || distance < existing.distance) byVisualKey.set(key, { ...normalized, distance });
			};
			rows.forEach(({ row, rect }) => {
				...
				if (beforeIntent) considerIntent({ ...beforeIntent, side: beforeIntent.side === 'inside' ? 'before' : beforeIntent.side }, beforeDistance);
				if (afterIntent) considerIntent(afterIntent, afterDistance);
			});
			return Array.from(byVisualKey.values()).reduce((best, candidate) =>
		...
		const confirmFolderOut = (_moveItems, onOk) => {
		...
		const applyCurrentDrop = (e) => {
			...
			const moveIds = draggingType === 'dialog' ? getDragPayloadIds(e) : [String(movedId)];
		...
		items.forEach(item => {
				...
				if (folderStatus.childCount) {
					...
					const applyFolderCollapsedDomState = (collapsed) => {
						...
						childRows.forEach(child => {
							child.hidden = false;
							child.classList.toggle('--folder-hidden', !!collapsed);
						});
						if (collapsed && childRows.some(child => isVisibleElement(child))) return false;
						visibleFolderChildrenCache = null;
						invalidateDropMetrics();
						_fitDialogDockHeight(false);
						_dialogControlLastSig = _getDialogControlStatusSig();
			...
			row.addEventListener('dragstart', (e) => {
				draggingId = item.id;
				draggingType = 'dialog';
				const moveGroupIds = _normalizeDialogControlMoveIds(_getDialogControlMoveGroupIds(item.id, items));
				draggingIds = new Set(moveGroupIds);
				...
				if (e.dataTransfer) {
					e.dataTransfer.effectAllowed = 'move';
					try { e.dataTransfer.setData(dragPayloadMime, JSON.stringify(moveGroupIds)); } catch {}
					try { e.dataTransfer.setData('text/plain', moveGroupIds.join('\n')); } catch {}
				}
			});
			row.addEventListener('dragend', () => {
				stopDragAutoScroll();
...
.dialog-control-context-item.--read{color:#bff3d0}
.dialog-control-context-item.--read svg{stroke:#5dc87e}
...
#anit-dialog-control-dock .dialog-control-chip.--folder-hidden{display:none!important}
...
<div id="anit_mini_toggle" class="mini-toggle" title="Показать панель (Ctrl+Alt+F)">
  <svg viewBox="0 0 402.577 402.577" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
	...
	if (_verBadge) _verBadge.textContent = 'v7.1.43';
