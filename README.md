...
Текущая версия: **8.0.15**.

Версия 8.0.15 выпускается на GitHub: Windows EXE, macOS Universal DMG и Chrome ZIP для ручной установки. Публикация в Chrome Web Store отложена. Во всех папках сохраняются настоящие строки и аватары Bitrix. Недостающие диалоги папки подгружает штатный сервис Bitrix в фоне, без принудительной прокрутки, блокировки интерфейса и лимита по количеству. Поиск использует штатную удалённую выдачу. В фильтрах доступны «По дате» (свежие сверху), «По цвету» и «Только непрочитанные». Цвета группируются в загруженном списке, закрепления остаются сверху; поиск сохраняет порядок Bitrix. Цветовой маркер можно назначить любому диалогу независимо от папки; перенос и перезапуск сохраняют его.
...
- Chrome: выполните `pnpm build:chrome` и загрузите папку `chrome-release/unpacked/` через «Загрузить распакованное расширение». Для публикации используйте `chrome-release/BX24_Chat_Sorter_Chrome_v8.0.15.zip`. Браузерная версия называется «Сортировщик чатов Bitrix24», подключает только выбранные HTTPS-порталы. [Установка и материалы Chrome Web Store](docs/chrome/README.md).
