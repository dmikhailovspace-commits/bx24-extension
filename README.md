...
Расширение PENA Agency для настольного Bitrix24. Добавляет папки, цветовые маркеры, поиск, фильтр непрочитанных, сохраняя нативную ленту чатов.

Текущая версия: **8.0.16**.

Версия 8.0.16 выпускается на GitHub: Windows EXE, macOS Universal DMG и Chrome ZIP для ручной установки. Публикация в Chrome Web Store отложена. Во всех папках сохраняются настоящие строки и аватары Bitrix. Недостающие диалоги папки подгружает штатный сервис Bitrix в фоне, без принудительной прокрутки, блокировки интерфейса и лимита по количеству. Поиск использует штатную удалённую выдачу. Пустое поле возвращает обычный список без нажатия крестика; непустой запрос сохраняется при открытии диалога. В фильтрах доступны «По дате» (свежие сверху), «По цвету» и «Только непрочитанные». Цвета группируются в загруженном списке, закрепления остаются сверху; поиск сохраняет порядок Bitrix. Цветовой маркер можно назначить любому диалогу независимо от папки; перенос и перезапуск сохраняют его.

Автообновление desktop: ежедневно в 10:00 на Windows и macOS; Windows также проверяет обновление при запуске через ярлык PENA. Канал main/update.json обновляется после проверенного релиза. Chrome ZIP обновляется вручную.
...
- Chrome: выполните `pnpm build:chrome` и загрузите папку `chrome-release/unpacked/` через «Загрузить распакованное расширение». Для публикации используйте `chrome-release/BX24_Chat_Sorter_Chrome_v8.0.16.zip`. Браузерная версия называется «Сортировщик чатов Bitrix24», подключает только выбранные HTTPS-порталы. [Установка и материалы Chrome Web Store](docs/chrome/README.md).
